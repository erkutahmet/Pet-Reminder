...
class HayvanViewController: UIViewController{
    ...
    @IBAction func suDuzenleButtonTiklandi(_ sender: Any){
        ...
        vc.title = "Hatırlatıcı Ayarla"
        vc.hatirlaticiDegerler = { (baslik, aciklama, tarih) in
            // istenilen yapıyı sıraya alma ve bildirim ana çekirdekte çalıştığı için yapmak zorundayız async olmasının nedeni de uygulama kullanırken dahi istenilen yapı çalışmasını istediğimiz için async kullanılır
            DispatchQueue.main.async {
                self.navigationController?.popToRootViewController(animated: true)
                let icerik = UNMutableNotificationContent()
                icerik.title = baslik
